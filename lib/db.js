import Database from "better-sqlite3";
import fs from "fs";
import path from "path";

// Prefer an explicit DATA_DIR, then Railway's injected volume mount path
// (present whenever a volume is attached), then the local data/ directory.
const DATA_DIR =
  process.env.DATA_DIR ||
  process.env.RAILWAY_VOLUME_MOUNT_PATH ||
  path.join(process.cwd(), "data");
const DB_PATH = path.join(DATA_DIR, "aotd.db");

let db;
let stmts;

/* Retention for the day-scoped vote tables. Every read of these three is
   `WHERE <key> = ?` against a key built from a single date, and none of them
   feed the lifetime figures on the stats board — so once their day has passed
   the rows are unreachable and only cost disk.

   ratings, vibes and guess_stats are deliberately NOT pruned: those do back
   lifetime totals, and dropping them would quietly walk the stats board
   backwards. Set VOTE_RETENTION_DAYS=0 to keep everything forever. */
const VOTE_RETENTION_DAYS = (() => {
  const raw = process.env.VOTE_RETENTION_DAYS;
  if (raw === undefined || raw === "") return 365;
  const parsed = Number(raw);
  return Number.isFinite(parsed) && parsed >= 0 ? parsed : 365;
})();

const PRUNABLE_VOTE_TABLES = [
  "playlist_votes",
  "matchup_votes",
  "soundtrack_votes",
];

/** Drop unreachable vote rows once at startup. Deploys restart the process
    often enough that a timer would be redundant. */
function pruneExpiredVotes(database) {
  if (!VOTE_RETENTION_DAYS) return;
  const cutoff = `-${VOTE_RETENTION_DAYS} days`;
  for (const table of PRUNABLE_VOTE_TABLES) {
    try {
      database
        .prepare(`DELETE FROM ${table} WHERE created_at < datetime('now', ?)`)
        .run(cutoff);
    } catch {
      // A pruning failure must never stop the site from opening its database
    }
  }
}

function getDb() {
  if (!db) {
    const dir = path.dirname(DB_PATH);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });

    db = new Database(DB_PATH);
    db.pragma("journal_mode = WAL");
    db.pragma("busy_timeout = 5000");

    db.exec(`
      CREATE TABLE IF NOT EXISTS ratings (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        album_key TEXT NOT NULL,
        rating INTEGER NOT NULL CHECK(rating >= 1 AND rating <= 10),
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );

      CREATE TABLE IF NOT EXISTS vibes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        album_key TEXT NOT NULL,
        vibe TEXT NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );

      CREATE TABLE IF NOT EXISTS guess_stats (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        puzzle_key TEXT NOT NULL,
        attempts INTEGER NOT NULL,
        solved INTEGER NOT NULL DEFAULT 0,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );

      CREATE INDEX IF NOT EXISTS idx_ratings_album ON ratings(album_key);
      CREATE INDEX IF NOT EXISTS idx_ratings_covering ON ratings(album_key, rating);
      CREATE INDEX IF NOT EXISTS idx_vibes_album ON vibes(album_key);
      CREATE INDEX IF NOT EXISTS idx_vibes_covering ON vibes(album_key, vibe);
      -- Site-wide "top vibes" groups by vibe alone. The covering index above
      -- leads with album_key, so that query could not use it and built a temp
      -- B-tree over every row instead; this one makes it an ordered scan.
      CREATE INDEX IF NOT EXISTS idx_vibes_vibe ON vibes(vibe);
      CREATE INDEX IF NOT EXISTS idx_guess_puzzle ON guess_stats(puzzle_key);
      CREATE INDEX IF NOT EXISTS idx_guess_covering ON guess_stats(puzzle_key, attempts, solved);

      CREATE TABLE IF NOT EXISTS playlist_votes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        album_key TEXT NOT NULL,
        vote INTEGER NOT NULL CHECK(vote IN (0, 1)),
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_playlist_album ON playlist_votes(album_key);
      CREATE INDEX IF NOT EXISTS idx_playlist_covering ON playlist_votes(album_key, vote);

      CREATE TABLE IF NOT EXISTS matchup_votes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        matchup_key TEXT NOT NULL,
        pick TEXT NOT NULL CHECK(pick IN ('A', 'B')),
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_matchup_key ON matchup_votes(matchup_key);
      CREATE INDEX IF NOT EXISTS idx_matchup_covering ON matchup_votes(matchup_key, pick);

      CREATE TABLE IF NOT EXISTS soundtrack_votes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        album_key TEXT NOT NULL,
        pick TEXT NOT NULL CHECK(pick IN ('game', 'film', 'tv')),
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_soundtrack_covering ON soundtrack_votes(album_key, pick);
    `);

    pruneExpiredVotes(db);

    // Cache prepared statements to avoid re-parsing SQL per request
    stmts = {
      addRating: db.prepare(
        "INSERT INTO ratings (album_key, rating) VALUES (?, ?)",
      ),
      getRatingDist: db.prepare(
        "SELECT rating, COUNT(*) as count FROM ratings WHERE album_key = ? GROUP BY rating",
      ),
      addVibe: db.prepare("INSERT INTO vibes (album_key, vibe) VALUES (?, ?)"),
      getVibeDist: db.prepare(
        "SELECT vibe, COUNT(*) as count FROM vibes WHERE album_key = ? GROUP BY vibe ORDER BY count DESC",
      ),
      addGuess: db.prepare(
        "INSERT INTO guess_stats (puzzle_key, attempts, solved) VALUES (?, ?, ?)",
      ),
      getGuessDist: db.prepare(
        "SELECT attempts, solved, COUNT(*) as count FROM guess_stats WHERE puzzle_key = ? GROUP BY attempts, solved",
      ),
      statsRatings: db.prepare(
        "SELECT COUNT(*) as totalRatings, AVG(rating) as avgRating FROM ratings",
      ),
      statsAlbums: db.prepare(
        "SELECT COUNT(DISTINCT album_key) as count FROM ratings",
      ),
      statsVibes: db.prepare(
        "SELECT vibe, COUNT(*) as count FROM vibes GROUP BY vibe ORDER BY count DESC LIMIT 5",
      ),
      statsPuzzles: db.prepare(
        "SELECT COUNT(*) as totalPlayed, SUM(solved) as totalSolved FROM guess_stats",
      ),
      addPlaylistVote: db.prepare(
        "INSERT INTO playlist_votes (album_key, vote) VALUES (?, ?)",
      ),
      getPlaylistDist: db.prepare(
        "SELECT vote, COUNT(*) as count FROM playlist_votes WHERE album_key = ? GROUP BY vote",
      ),
      addMatchupVote: db.prepare(
        "INSERT INTO matchup_votes (matchup_key, pick) VALUES (?, ?)",
      ),
      getMatchupDist: db.prepare(
        "SELECT pick, COUNT(*) as count FROM matchup_votes WHERE matchup_key = ? GROUP BY pick",
      ),
      addSoundtrackVote: db.prepare(
        "INSERT INTO soundtrack_votes (album_key, pick) VALUES (?, ?)",
      ),
      getSoundtrackDist: db.prepare(
        "SELECT pick, COUNT(*) as count FROM soundtrack_votes WHERE album_key = ? GROUP BY pick",
      ),
      getSoundtrackHistory: db.prepare(
        "SELECT album_key, pick, COUNT(*) as count FROM soundtrack_votes WHERE album_key >= ? GROUP BY album_key, pick",
      ),
    };
  }
  return db;
}

export function addRating(albumKey, rating) {
  getDb();
  stmts.addRating.run(albumKey, rating);
}

export function getRatingDistribution(albumKey) {
  getDb();
  const rows = stmts.getRatingDist.all(albumKey);

  const distribution = {};
  for (let i = 1; i <= 10; i++) distribution[i] = 0;
  for (const row of rows) distribution[row.rating] = row.count;

  const total = Object.values(distribution).reduce((a, b) => a + b, 0);
  const avg =
    total > 0
      ? rows.reduce((sum, r) => sum + r.rating * r.count, 0) / total
      : 0;

  return { distribution, total, average: Math.round(avg * 10) / 10 };
}

export function addVibe(albumKey, vibe) {
  getDb();
  stmts.addVibe.run(albumKey, vibe);
}

export function getVibeDistribution(albumKey) {
  getDb();
  const rows = stmts.getVibeDist.all(albumKey);

  const distribution = {};
  let total = 0;
  for (const row of rows) {
    distribution[row.vibe] = row.count;
    total += row.count;
  }

  return { distribution, total };
}

export function addGuessStat(puzzleKey, attempts, solved) {
  getDb();
  stmts.addGuess.run(puzzleKey, attempts, solved ? 1 : 0);
}

let statsCache = null;
let statsCacheTime = 0;
const STATS_TTL = 300000; // 5 min — stats don't need to be real-time

export function getSiteStats() {
  const now = Date.now();
  if (statsCache && now - statsCacheTime < STATS_TTL) return statsCache;
  getDb();
  const ratingRow = stmts.statsRatings.get();
  const albumsRated = stmts.statsAlbums.get();
  const topVibes = stmts.statsVibes.all();
  const puzzleRow = stmts.statsPuzzles.get();
  statsCache = {
    totalRatings: ratingRow.totalRatings,
    avgRating: ratingRow.avgRating
      ? Math.round(ratingRow.avgRating * 10) / 10
      : 0,
    albumsRated: albumsRated.count,
    topVibes,
    totalPuzzlePlayed: puzzleRow.totalPlayed,
    totalPuzzleSolved: puzzleRow.totalSolved || 0,
  };
  statsCacheTime = now;
  return statsCache;
}

export function addPlaylistVote(albumKey, vote) {
  getDb();
  stmts.addPlaylistVote.run(albumKey, vote ? 1 : 0);
}

export function getPlaylistDistribution(albumKey) {
  getDb();
  const rows = stmts.getPlaylistDist.all(albumKey);
  let yes = 0,
    no = 0;
  for (const row of rows) {
    if (row.vote === 1) yes = row.count;
    else no = row.count;
  }
  return { yes, no, total: yes + no };
}

export function getGuessStats(puzzleKey) {
  getDb();
  const rows = stmts.getGuessDist.all(puzzleKey);

  let totalPlayers = 0;
  let totalSolved = 0;
  const attemptDist = {};

  for (const row of rows) {
    totalPlayers += row.count;
    if (row.solved) {
      totalSolved += row.count;
      attemptDist[row.attempts] = (attemptDist[row.attempts] || 0) + row.count;
    }
  }

  return { totalPlayers, totalSolved, attemptDist };
}

export function addMatchupVote(matchupKey, pick) {
  getDb();
  stmts.addMatchupVote.run(matchupKey, pick);
}

export function getMatchupDistribution(matchupKey) {
  getDb();
  const rows = stmts.getMatchupDist.all(matchupKey);
  let a = 0,
    b = 0;
  for (const row of rows) {
    if (row.pick === "A") a = row.count;
    else b = row.count;
  }
  return { a, b, total: a + b };
}

export function addSoundtrackVote(albumKey, pick) {
  getDb();
  stmts.addSoundtrackVote.run(albumKey, pick);
}

export function getSoundtrackDistribution(albumKey) {
  getDb();
  const rows = stmts.getSoundtrackDist.all(albumKey);
  const counts = { game: 0, film: 0, tv: 0 };
  for (const row of rows) {
    if (row.pick in counts) counts[row.pick] = row.count;
  }
  return { ...counts, total: counts.game + counts.film + counts.tv };
}

/**
 * Every day's cue split from sinceKey (a YYYY-MM-DD album_key) forward, keyed
 * by date. The Archive joins it against the albums it already lists, so the
 * corner's daily vote leaves a trail instead of resetting at midnight.
 * The covering index on (album_key, pick) already serves this shape.
 */
export function getSoundtrackHistory(sinceKey) {
  getDb();
  const rows = stmts.getSoundtrackHistory.all(sinceKey);
  const history = {};
  for (const row of rows) {
    if (!history[row.album_key]) {
      history[row.album_key] = { game: 0, film: 0, tv: 0, total: 0 };
    }
    if (row.pick in history[row.album_key]) {
      history[row.album_key][row.pick] = row.count;
      history[row.album_key].total += row.count;
    }
  }
  return history;
}

/**
 * Write a consistent snapshot of the live database to destPath.
 * Uses better-sqlite3's online backup (safe under concurrent WAL writes).
 * Used by the token-gated /api/backup route.
 */
export async function backupDatabase(destPath) {
  const database = getDb();
  await database.backup(destPath);
}
