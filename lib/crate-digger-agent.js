import fs from "node:fs/promises";
import path from "node:path";
import OpenAI from "openai";
import {
  getChatScopeBoundary,
  moderateChatPrompt,
} from "@/lib/chat-moderation";

export const DEFAULT_CRATE_DIGGER_MODEL = "gemma3:4b";
export const DEFAULT_CRATE_DIGGER_PROVIDER = "ollama";

const DEFAULT_OPENAI_MODEL = "gpt-5.4-mini";
const DEFAULT_OLLAMA_HOST = "http://127.0.0.1:11434";
const MAX_REPLY_CHARS = 1800;
const MAX_FILE_RESULTS = 4;
const MAX_LOCAL_KNOWLEDGE_RESULTS = 4;
const KNOWLEDGE_DIR = path.join(process.cwd(), "public", "agent-knowledge");
const ALWAYS_INCLUDED_KNOWLEDGE_FILES = new Set([
  "site-voice.md",
  "source-policy.md",
  "conversation-style.md",
  "safety-and-limits.md",
]);
const SAFETY_CITATION_URL = "/agent-knowledge/safety-and-limits.md";
const STOP_WORDS = new Set([
  "about",
  "after",
  "again",
  "album",
  "also",
  "and",
  "are",
  "artist",
  "best",
  "but",
  "for",
  "from",
  "game",
  "have",
  "into",
  "just",
  "like",
  "more",
  "music",
  "not",
  "that",
  "the",
  "their",
  "them",
  "this",
  "today",
  "what",
  "with",
  "would",
  "your",
]);
const KNOWLEDGE_INTENT_BOOSTS = [
  {
    filename: "music-analysis-lenses.md",
    patterns: [
      /\b(?:production|mix|arrangement|melody|harmony|drums|bass|guitar|synth|vocals?|lyrics|hook|chorus|verse|riff|texture)\b/i,
    ],
  },
  {
    filename: "soundtrack-taxonomy.md",
    patterns: [
      /\b(?:soundtrack|ost|score|theme song|opening theme|closing theme|credits music)\b/i,
    ],
  },
  {
    filename: "scene-and-era-bridges.md",
    patterns: [
      /\b(?:scene|era|movement|revival|blog era|college radio|mixtape|internet|subgenre|underground|mainstream|trend)\b/i,
    ],
  },
  {
    filename: "artist-context-lenses.md",
    patterns: [
      /\b(?:artist|career|discography|producer|label|collaborator|feature|credits|influence|context)\b/i,
    ],
  },
  {
    filename: "recommendation-bridge-playbook.md",
    patterns: [
      /\b(?:recommend|similar|like this|what next|where do i start|starter|entry point|deep cut|accessible)\b/i,
    ],
  },
  {
    filename: "forum-debate-moves.md",
    patterns: [
      /\b(?:opinion|hot take|overrated|underrated|defend|argue|debate|unpopular|wrong about)\b/i,
    ],
  },
  {
    filename: "artist-arc-case-studies.md",
    patterns: [
      /\b(?:breakthrough|run|imperial phase|comeback|pivot|reinvention|late career|discography|phase|era)\b/i,
    ],
  },
  {
    filename: "scene-pocket-guides.md",
    patterns: [
      /\b(?:scene|era|movement|revival|city|downtown|madchester|shoegaze|trip-hop|post-punk|blog era|blog-era|indie sleaze)\b/i,
    ],
  },
];
const CHAT_REPLY_MODES = [
  {
    key: "fact-check",
    patterns: [
      /\b(?:is this true|is that true|did\b|when\b|what year|release date|chart|credits|producer|label|sample|controversy|what happened|accurate|verify)\b/i,
    ],
    guidance: [
      "Reply mode: fact-check.",
      "Lead with the answer or the safest version of the answer in the first sentence.",
      "If you are not sure, say that early instead of improvising fake certainty.",
      "Keep the tone human, but do not get cute with facts.",
    ],
  },
  {
    key: "recommendation",
    patterns: [
      /\b(?:recommend|recommendation|similar|what next|where do i start|starter|entry point|put me on|three albums|album recs?)\b/i,
    ],
    guidance: [
      "Reply mode: recommendation.",
      "Give 1 to 3 picks max unless the user asked for more.",
      "Make each pick do a different job instead of giving three near-duplicates.",
      "Explain each pick with concrete texture, mood, or craft detail, not just genre labels.",
    ],
  },
  {
    key: "comparison",
    patterns: [
      /\b(?:compare|comparison|vs\.?|versus|difference between|better than|more than|less than)\b/i,
    ],
    guidance: [
      "Reply mode: comparison.",
      "Name the hinge quickly: songwriting, production, scale, vocal framing, rhythm, scene, or emotional temperature.",
      "Make the distinction sharp enough that the user can actually hear it.",
    ],
  },
  {
    key: "music-detail",
    patterns: [
      /\b(?:production|mix|arrangement|drums|bass|guitar|synth|keys|piano|vocals?|lyrics|hook|chorus|verse|melody|harmony|riff|texture|sound design)\b/i,
    ],
    guidance: [
      "Reply mode: music detail.",
      "Stay on the audible stuff: arrangement, performance, texture, rhythm, tone, sequencing, or writing.",
      "Do not hide behind 'vibes' when you can point to an actual musical detail.",
    ],
  },
  {
    key: "debate",
    patterns: [
      /\b(?:hot take|overrated|underrated|defend|unpopular|wrong about|why do people|am i crazy)\b/i,
    ],
    guidance: [
      "Reply mode: debate.",
      "Take a clear position fast.",
      "A little dry humor is fine, but it should come with an actual argument.",
      "Do not hedge the whole answer into mush.",
    ],
  },
  {
    key: "context",
    patterns: [
      /\b(?:context|scene|era|movement|history|historical|influence|discography|why is|why was|important)\b/i,
    ],
    guidance: [
      "Reply mode: context.",
      "Give the one-paragraph version first.",
      "Start with the hinge point before adding side detail.",
      "Anchor claims to the actual year and scene instead of flattening history into modern shorthand.",
    ],
  },
  {
    key: "chatty",
    patterns: [
      /\b(?:i think|i feel|i love|i hate|i'm obsessed|im obsessed|favorite|favourite|can't stand|cant stand)\b/i,
    ],
    guidance: [
      "Reply mode: chatty thread reply.",
      "React like a person first in one line, then give the useful take.",
      "One short follow-up is fine if it feels natural, but do not force it.",
    ],
  },
];
const MODE_KNOWLEDGE_PREFERENCES = {
  default: ["album-club-canon.md", "music-analysis-lenses.md"],
  "fact-check": [
    "source-policy.md",
    "artist-context-lenses.md",
    "scene-pocket-guides.md",
  ],
  recommendation: [
    "recommendation-style.md",
    "recommendation-bridge-playbook.md",
    "artist-arc-case-studies.md",
    "album-club-canon.md",
  ],
  comparison: [
    "music-analysis-lenses.md",
    "artist-context-lenses.md",
    "artist-arc-case-studies.md",
    "recommendation-bridge-playbook.md",
  ],
  "music-detail": [
    "music-analysis-lenses.md",
    "artist-context-lenses.md",
    "recommendation-style.md",
    "scene-pocket-guides.md",
  ],
  debate: [
    "forum-debate-moves.md",
    "album-club-canon.md",
    "scene-and-era-bridges.md",
    "music-analysis-lenses.md",
  ],
  context: [
    "artist-context-lenses.md",
    "artist-arc-case-studies.md",
    "scene-and-era-bridges.md",
    "scene-pocket-guides.md",
  ],
  chatty: [
    "conversation-style.md",
    "site-voice.md",
    "album-club-canon.md",
    "forum-debate-moves.md",
  ],
};

let warnedNoVectorStore = false;
let knowledgePackPromise = null;

function makeAgentError(message, status, code) {
  const error = new Error(message);
  error.status = status;
  error.code = code;
  return error;
}

function normalizeProvider(value) {
  if (typeof value !== "string") return null;
  const normalized = value.trim().toLowerCase();
  if (normalized === "ollama" || normalized === "openai") return normalized;
  return null;
}

function getExplicitCrateDiggerProvider() {
  return (
    normalizeProvider(process.env.CRATE_DIGGER_PROVIDER) ||
    normalizeProvider(process.env.AI_PROVIDER)
  );
}

function isDevelopmentRuntime() {
  return process.env.NODE_ENV === "development";
}

export function getCrateDiggerOllamaHost() {
  const configuredHost =
    typeof process.env.OLLAMA_HOST === "string"
      ? process.env.OLLAMA_HOST.trim()
      : "";

  if (configuredHost) {
    return configuredHost.replace(/\/$/, "");
  }

  return isDevelopmentRuntime() ? DEFAULT_OLLAMA_HOST : null;
}

export function getCrateDiggerRuntimeStatus() {
  const explicitProvider = getExplicitCrateDiggerProvider();
  const hasOpenAIKey = Boolean(process.env.OPENAI_API_KEY);
  const vectorStoreId = process.env.OPENAI_VECTOR_STORE_ID;
  const ollamaHost = getCrateDiggerOllamaHost();

  let provider = explicitProvider;

  if (!provider) {
    if (isDevelopmentRuntime()) {
      provider = DEFAULT_CRATE_DIGGER_PROVIDER;
    } else if (hasOpenAIKey) {
      provider = "openai";
    } else if (ollamaHost) {
      provider = "ollama";
    }
  }

  if (!provider) {
    return {
      available: false,
      provider: null,
      model: null,
      ollamaHost: null,
      enabledTools: { web: false, files: false },
      reason:
        "Chat Booth is unavailable on this deployment right now. Local Ollama is only the development default; the live site needs a hosted provider or a remote Ollama server.",
      code: "CHAT_UNAVAILABLE",
    };
  }

  const model = getCrateDiggerModel(provider);

  if (provider === "openai" && !hasOpenAIKey) {
    return {
      available: false,
      provider,
      model,
      ollamaHost: null,
      enabledTools: { web: false, files: false },
      reason:
        "Chat Booth is unavailable right now. The hosted chat provider is selected, but it is not configured for this deployment.",
      code: "OPENAI_MISSING_KEY",
    };
  }

  if (provider === "ollama" && !ollamaHost) {
    return {
      available: false,
      provider,
      model,
      ollamaHost: null,
      enabledTools: { web: false, files: false },
      reason:
        "Chat Booth is unavailable on this deployment right now. The free local Ollama mode only auto-connects in development; production needs a remote Ollama host or a hosted provider.",
      code: "OLLAMA_HOST_MISSING",
    };
  }

  const tools = buildCrateDiggerTools({ provider, vectorStoreId });

  return {
    available: true,
    provider,
    model,
    ollamaHost: provider === "ollama" ? ollamaHost : null,
    enabledTools: getEnabledToolLabels({ provider, tools }),
    reason: "",
    code: "CHAT_AVAILABLE",
  };
}

export async function getCrateDiggerRuntimeStatusWithHealthCheck() {
  const status = getCrateDiggerRuntimeStatus();

  if (!status.available || status.provider !== "ollama" || !status.ollamaHost) {
    return status;
  }

  try {
    const response = await fetch(`${status.ollamaHost}/api/tags`, {
      signal: AbortSignal.timeout(2000),
    });

    if (response.ok) {
      return status;
    }
  } catch {}

  return {
    ...status,
    available: false,
    enabledTools: { web: false, files: false },
    reason: `Crate Digger is set to use the free local Ollama mode, but nothing is listening at ${status.ollamaHost}. Install Ollama, then run: ollama pull ${status.model}`,
    code: "OLLAMA_UNAVAILABLE",
  };
}

export function getCrateDiggerProvider() {
  return getCrateDiggerRuntimeStatus().provider;
}

export function getCrateDiggerModel(provider) {
  if (provider === "openai") {
    return process.env.OPENAI_MODEL || DEFAULT_OPENAI_MODEL;
  }

  return process.env.OLLAMA_MODEL || DEFAULT_CRATE_DIGGER_MODEL;
}

export function buildCrateDiggerInstructions(album) {
  return buildCrateDiggerInstructionsForMessages(album, []);
}

function getConversationMode(messages) {
  const latestUserMessage = getLatestUserMessage(messages);

  for (const mode of CHAT_REPLY_MODES) {
    if (mode.patterns.some((pattern) => pattern.test(latestUserMessage))) {
      return mode;
    }
  }

  return {
    key: "default",
    guidance: [
      "Reply mode: default forum answer.",
      "Give one sharp answer, one memorable detail, and stop before it turns into homework.",
    ],
  };
}

function buildCrateDiggerInstructionsForMessages(album, messages) {
  const mode = getConversationMode(messages);

  return [
    "You are Crate Digger, the Album Of The Day Club chat agent.",
    "You are a daily companion for album nerds and curious listeners.",
    "Your lane is music only: albums, songs, artists, scenes, production, lyrics, credits, recommendations, discographies, and soundtrack music as music.",
    `Today's album is "${album.title}" by ${album.artist} (${album.year}, ${album.genre}).`,
    "You are not a human being. Do not imply you have a body, ears, memories, a private life, or firsthand experience.",
    "If a user asks what you personally listened to, felt, remembered, played, or lived through, answer honestly that you are a model going off patterns, context, and available sources.",
    "Answer the user's actual question first. Tie back to today's album only when it helps.",
    "Sound like a real person in a good forum thread, not a customer-support article.",
    "Be warm, opinionated, concise, and specific. No gatekeeping. No fake certainty.",
    "Use contractions and natural rhythm. It is fine to be a little dry, playful, or lightly snarky when it feels earned.",
    "The snark should punch up at trends, cliches, or overhyped ideas, not at the user.",
    "Have taste. It is okay to say something is clunky, too polished, trying too hard, or kind of corny if you explain why.",
    "When the user is chatting rather than fact-checking, react like a person first, then answer, then keep the thread alive with one short follow-up only when natural.",
    "Keep most replies under 140 words unless the user asks for depth.",
    "Default to one or two short paragraphs, not a wall of text.",
    "Avoid lists unless the user asks for a list.",
    "Do not restate the user's question before answering it.",
    "Avoid repetitive openers like 'absolutely,' 'great question,' 'let's dig in,' 'totally,' or 'for sure' unless one genuinely fits the moment.",
    "Vary your cadence. Not every reply should sound like the same polite assistant wearing different records.",
    "The first sentence should usually contain either the take, the answer, or the reaction. Minimize throat-clearing.",
    "If the user asks for general film, TV, game, celebrity, or pop-culture talk that is not clearly about the music, say that is outside your lane and redirect to a music-framed version of the question.",
    "Use model knowledge for taste, mood, recommendations, and broad context.",
    "Do not pretend to know everything. If you are unsure, say so plainly and say what you would want to verify.",
    "When talking about scenes or eras, anchor claims to the album's actual year and context. Do not slap later internet-era labels onto older records unless you clearly say you are describing later reception.",
    "Do not help create, endorse, or justify racist, sexist, homophobic, transphobic, antisemitic, ableist, or other hateful content. Set a brief boundary and redirect.",
    "Do not quote long lyrics, reviews, articles, or copyrighted passages. Summarize in your own words.",
    ...mode.guidance,
  ].join("\n");
}

function getLatestUserMessage(messages) {
  return (
    [...messages].reverse().find((message) => message.role === "user")?.content ||
    ""
  );
}

function buildLocalModeHints(album, messages) {
  const latestUserMessage = getLatestUserMessage(messages).toLowerCase();
  const hints = [];
  const mode = getConversationMode(messages);

  hints.push(...mode.guidance);

  if (/\b(?:scene|era|movement|context)\b/.test(latestUserMessage)) {
    hints.push(
      `Historical anchor: this album came out in ${album.year}. Do not mention blog era, internet scenes, or online music communities unless that actually fits the year and context.`,
    );
  }

  if (/\b(?:producer|credits|label|sample)\b/.test(latestUserMessage)) {
    hints.push(
      "For credits, labels, producers, and samples, be careful: if you are not sure, say you would verify rather than guessing.",
    );
  }

  return hints;
}

function repairHistoricalOverreach({ album, messages, response }) {
  const latestUserMessage = getLatestUserMessage(messages).toLowerCase();

  if (
    album.year < 1990 &&
    /\b(?:scene|era|movement|context)\b/.test(latestUserMessage) &&
    /\b(?:blog[- ]era|internet scenes?|online music communities?)\b/i.test(
      response.reply,
    )
  ) {
    return {
      ...response,
      reply: `I don't want to fake the scene history here. The safer read is that "${album.title}" belongs to its ${album.year} context around ${album.artist} and the broader ${album.genre.toLowerCase()} world, but I would verify the exact scene or label framing before getting more specific.`,
    };
  }

  return response;
}

function buildBoundaryResponse({
  provider,
  vectorStoreId,
  reply,
  citations = [],
  usedTools = { web: false, files: false },
}) {
  const tools =
    provider === "openai"
      ? buildCrateDiggerTools({ provider, vectorStoreId })
      : [];

  return {
    reply,
    citations,
    usedTools,
    enabledTools: getEnabledToolLabels({ provider, tools }),
    provider,
  };
}

function getGuardrailResponse({ messages, provider, vectorStoreId }) {
  const latestUserMessage = getLatestUserMessage(messages).trim();
  if (!latestUserMessage) return null;

  const moderation = moderateChatPrompt(latestUserMessage);
  if (!moderation.ok && moderation.reply) {
    return buildBoundaryResponse({
      provider,
      vectorStoreId,
      reply: moderation.reply,
      citations: [
        {
          type: "file",
          title: "Safety And Limits",
          url: SAFETY_CITATION_URL,
        },
      ],
      usedTools: { web: false, files: true },
    });
  }

  const scopeBoundary = getChatScopeBoundary(latestUserMessage);
  if (!scopeBoundary.inScope && scopeBoundary.reply) {
    return buildBoundaryResponse({
      provider,
      vectorStoreId,
      reply: scopeBoundary.reply,
    });
  }

  return null;
}

async function loadKnowledgePack() {
  if (!knowledgePackPromise) {
    knowledgePackPromise = (async () => {
      const entries = await fs.readdir(KNOWLEDGE_DIR, { withFileTypes: true });
      const files = entries
        .filter((entry) => entry.isFile() && entry.name.endsWith(".md"))
        .map((entry) => entry.name)
        .sort();

      const docs = await Promise.all(
        files.map(async (filename) => {
          const fullPath = path.join(KNOWLEDGE_DIR, filename);
          const text = await fs.readFile(fullPath, "utf8");
          const heading = text.match(/^#\s+(.+)$/m);
          const title = heading
            ? heading[1].trim()
            : filename.replace(/\.md$/i, "");
          return {
            filename,
            title,
            url: `/agent-knowledge/${filename}`,
            text,
            haystack: `${filename}\n${title}\n${text}`.toLowerCase(),
          };
        }),
      );

      return docs;
    })().catch((error) => {
      knowledgePackPromise = null;
      throw error;
    });
  }

  return knowledgePackPromise;
}

function tokenize(text) {
  return [
    ...new Set(
      String(text)
        .toLowerCase()
        .match(/[a-z0-9]+/g)
        ?.filter((token) => token.length > 2 && !STOP_WORDS.has(token)) || [],
    ),
  ];
}

function getKnowledgeIntentBoost(doc, query) {
  const boost = KNOWLEDGE_INTENT_BOOSTS.find(
    (entry) => entry.filename === doc.filename,
  );
  if (!boost) return 0;

  return boost.patterns.reduce(
    (sum, pattern) => sum + (pattern.test(query) ? 3 : 0),
    0,
  );
}

function getModeKnowledgePreferenceBoost(doc, messages) {
  const mode = getConversationMode(messages);
  const preferred =
    MODE_KNOWLEDGE_PREFERENCES[mode.key] ||
    MODE_KNOWLEDGE_PREFERENCES.default ||
    [];

  return preferred.includes(doc.filename) ? 5 : 0;
}

function selectRelevantKnowledgeDocs(docs, messages, album) {
  const latestUserMessage =
    [...messages].reverse().find((message) => message.role === "user")
      ?.content || "";
  const query = `${latestUserMessage} ${album.title} ${album.artist} ${album.genre}`;
  const tokens = tokenize(query);
  const lowerQuery = query.toLowerCase();

  const scored = docs
    .map((doc) => ({
      doc,
      score:
        tokens.reduce(
          (sum, token) => sum + (doc.haystack.includes(token) ? 1 : 0),
          0,
        ) +
        getKnowledgeIntentBoost(doc, lowerQuery) +
        getModeKnowledgePreferenceBoost(doc, messages),
    }))
    .sort(
      (a, b) => b.score - a.score || a.doc.title.localeCompare(b.doc.title),
    );

  const selected = scored
    .filter((item) => item.score > 0)
    .slice(0, MAX_LOCAL_KNOWLEDGE_RESULTS)
    .map((item) => item.doc);

  if (selected.length > 0) return selected;

  return docs.slice(0, MAX_LOCAL_KNOWLEDGE_RESULTS);
}

function selectPromptKnowledgeDocs(docs, messages, album) {
  const alwaysIncluded = docs.filter((doc) =>
    ALWAYS_INCLUDED_KNOWLEDGE_FILES.has(doc.filename),
  );
  const topicalDocs = selectRelevantKnowledgeDocs(
    docs.filter((doc) => !ALWAYS_INCLUDED_KNOWLEDGE_FILES.has(doc.filename)),
    messages,
    album,
  );

  return [
    ...new Map(
      [...alwaysIncluded, ...topicalDocs].map((doc) => [doc.filename, doc]),
    ).values(),
  ];
}

function buildKnowledgeContext(docs) {
  return docs.map((doc) => `## ${doc.title}\n${doc.text.trim()}`).join("\n\n");
}

export function buildCrateDiggerTools({ provider, vectorStoreId } = {}) {
  if (provider !== "openai") {
    return [];
  }

  const tools = [
    {
      type: "web_search",
      search_context_size: "medium",
      user_location: {
        type: "approximate",
        country: "US",
        timezone: "America/Denver",
      },
    },
  ];

  if (vectorStoreId) {
    tools.unshift({
      type: "file_search",
      vector_store_ids: [vectorStoreId],
      max_num_results: MAX_FILE_RESULTS,
    });
  } else if (!warnedNoVectorStore) {
    warnedNoVectorStore = true;
    console.info(
      "Crate Digger file search disabled: OPENAI_VECTOR_STORE_ID is not set.",
    );
  }

  return tools;
}

export function getEnabledToolLabels({ provider, tools }) {
  if (provider === "openai") {
    return {
      web: tools.some((tool) => tool.type === "web_search"),
      files: tools.some((tool) => tool.type === "file_search"),
    };
  }

  if (provider === "ollama") {
    return { web: false, files: true };
  }

  return { web: false, files: false };
}

function safeCitationUrl(url) {
  if (typeof url !== "string") return null;
  if (url.startsWith("https://") || url.startsWith("http://")) return url;
  if (url.startsWith("/agent-knowledge/")) return url;
  return null;
}

function titleFromUrl(url) {
  try {
    const parsed = new URL(url, "https://littlealbumclub.net");
    return parsed.hostname || parsed.pathname;
  } catch {
    return "Source";
  }
}

function addCitation(citations, seen, citation) {
  const url = safeCitationUrl(citation.url);
  const title =
    typeof citation.title === "string" && citation.title.trim()
      ? citation.title.trim()
      : url
        ? titleFromUrl(url)
        : "Source";
  const type = citation.type === "file" ? "file" : "web";
  const key = `${type}:${url || citation.fileId || title}`;

  if (seen.has(key)) return;
  seen.add(key);

  citations.push({
    type,
    title,
    url,
    fileId: typeof citation.fileId === "string" ? citation.fileId : undefined,
  });
}

function extractOpenAIOutputParts(response) {
  const textParts = [];
  const citations = [];
  const seen = new Set();
  const usedTools = { web: false, files: false };

  for (const item of response.output || []) {
    if (item?.type === "web_search_call") {
      usedTools.web = true;
      const sources = item.action?.sources || [];
      for (const source of sources) {
        addCitation(citations, seen, {
          type: "web",
          url: source.url,
        });
      }
    }

    if (item?.type === "file_search_call") {
      usedTools.files = true;
      for (const result of item.results || []) {
        addCitation(citations, seen, {
          type: "file",
          title: result.attributes?.title || result.filename,
          url: result.attributes?.source_url,
          fileId: result.file_id,
        });
      }
    }

    if (item?.type !== "message" || !Array.isArray(item.content)) continue;

    for (const content of item.content) {
      if (content?.type === "output_text" && typeof content.text === "string") {
        textParts.push(content.text);

        for (const annotation of content.annotations || []) {
          if (annotation.type === "url_citation") {
            addCitation(citations, seen, {
              type: "web",
              title: annotation.title,
              url: annotation.url,
            });
          }
          if (annotation.type === "file_citation") {
            addCitation(citations, seen, {
              type: "file",
              title: annotation.filename,
              url: `/agent-knowledge/${annotation.filename}`,
              fileId: annotation.file_id,
            });
          }
        }
      }

      if (content?.type === "refusal" && typeof content.refusal === "string") {
        textParts.push(content.refusal);
      }
    }
  }

  const reply =
    typeof response.output_text === "string" && response.output_text.trim()
      ? response.output_text.trim()
      : textParts.join("\n").trim();

  return {
    reply: reply.slice(0, MAX_REPLY_CHARS),
    citations,
    usedTools,
  };
}

async function createOpenAICrateDiggerResponse({
  apiKey,
  album,
  messages,
  model,
  vectorStoreId,
}) {
  if (!apiKey) {
    throw makeAgentError(
      "OpenAI mode is enabled, but OPENAI_API_KEY is missing.",
      503,
      "OPENAI_MISSING_KEY",
    );
  }

  const client = new OpenAI({ apiKey, timeout: 20000 });
  const tools = buildCrateDiggerTools({
    provider: "openai",
    vectorStoreId,
  });

  const response = await client.responses.create({
    model,
    instructions: buildCrateDiggerInstructionsForMessages(album, messages),
    input: messages.map((message) => ({
      role: message.role,
      content: message.content,
    })),
    tools,
    tool_choice: "auto",
    include: ["web_search_call.action.sources", "file_search_call.results"],
    max_output_tokens: 600,
    store: false,
  });

  return {
    ...extractOpenAIOutputParts(response),
    enabledTools: getEnabledToolLabels({ provider: "openai", tools }),
    provider: "openai",
  };
}

async function createOllamaCrateDiggerResponse({
  album,
  messages,
  model,
  ollamaHost,
}) {
  const knowledgeDocs = await loadKnowledgePack();
  const promptDocs = selectPromptKnowledgeDocs(knowledgeDocs, messages, album);
  const localModeHints = buildLocalModeHints(album, messages);

  if (!ollamaHost) {
    throw makeAgentError(
      "Chat Booth is unavailable on this deployment right now.",
      503,
      "OLLAMA_HOST_MISSING",
    );
  }

  const response = await fetch(`${ollamaHost}/api/chat`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      model,
      stream: false,
      messages: [
        {
          role: "system",
          content: [
            buildCrateDiggerInstructionsForMessages(album, messages),
            "You are in local free mode. There is no live web search in this mode.",
            "If you are unsure about a factual claim, say that you would verify it instead of guessing.",
            ...localModeHints,
            "Use the Album Club knowledge pack below for voice, conversation style, listening language, recommendation patterns, and music history/context.",
            buildKnowledgeContext(promptDocs),
          ].join("\n\n"),
        },
        ...messages,
      ],
      options: {
        temperature: 0.9,
        top_p: 0.95,
        repeat_penalty: 1.08,
      },
    }),
    signal: AbortSignal.timeout(30000),
  });

  let data = null;
  try {
    data = await response.json();
  } catch {}

  if (!response.ok) {
    const message = data?.error || "The local model failed to respond.";

    if (response.status === 404) {
      throw makeAgentError(
        `Crate Digger is set to use Ollama, but the model "${model}" is not available yet. Install Ollama, then run: ollama pull ${model}`,
        503,
        "OLLAMA_MODEL_MISSING",
      );
    }

    throw makeAgentError(message, 502, "OLLAMA_ERROR");
  }

  const reply =
    typeof data?.message?.content === "string"
      ? data.message.content.trim().slice(0, MAX_REPLY_CHARS)
      : "";

  if (!reply) {
    throw makeAgentError(
      "The local model did not send a reply.",
      502,
      "OLLAMA_EMPTY_REPLY",
    );
  }

  return repairHistoricalOverreach({
    album,
    messages,
    response: {
    reply,
    citations: promptDocs.map((doc) => ({
      type: "file",
      title: doc.title,
      url: doc.url,
    })),
    usedTools: { web: false, files: true },
    enabledTools: getEnabledToolLabels({ provider: "ollama", tools: [] }),
    provider: "ollama",
    },
  });
}

export async function createCrateDiggerResponse({
  apiKey,
  album,
  messages,
  provider = getCrateDiggerProvider(),
  model,
  vectorStoreId,
}) {
  const runtimeStatus = getCrateDiggerRuntimeStatus();

  if (!runtimeStatus.available || !runtimeStatus.provider) {
    throw makeAgentError(
      runtimeStatus.reason,
      503,
      runtimeStatus.code || "CHAT_UNAVAILABLE",
    );
  }

  const resolvedProvider = provider || runtimeStatus.provider;
  const resolvedModel = model || runtimeStatus.model;
  const resolvedVectorStoreId =
    typeof vectorStoreId === "string"
      ? vectorStoreId
      : process.env.OPENAI_VECTOR_STORE_ID;

  const guardrailResponse = getGuardrailResponse({
    messages,
    provider: resolvedProvider,
    vectorStoreId: resolvedVectorStoreId,
  });
  if (guardrailResponse) {
    return guardrailResponse;
  }

  if (resolvedProvider === "openai") {
    return createOpenAICrateDiggerResponse({
      apiKey,
      album,
      messages,
      model: resolvedModel,
      vectorStoreId: resolvedVectorStoreId,
    });
  }

  try {
    return await createOllamaCrateDiggerResponse({
      album,
      messages,
      model: resolvedModel,
      ollamaHost: runtimeStatus.ollamaHost,
    });
  } catch (error) {
    if (error?.name === "TimeoutError" || error?.name === "AbortError") {
      throw makeAgentError(
        "Crate Digger could not reach the local model in time. Make sure Ollama is running and try again.",
        503,
        "OLLAMA_TIMEOUT",
      );
    }

    if (
      error?.cause?.code === "ECONNREFUSED" ||
      error?.code === "ECONNREFUSED"
    ) {
      throw makeAgentError(
        `Crate Digger is set to use the free local Ollama mode, but nothing is listening at ${runtimeStatus.ollamaHost}. Install Ollama, then run: ollama pull ${resolvedModel}`,
        503,
        "OLLAMA_UNAVAILABLE",
      );
    }

    throw error;
  }
}
