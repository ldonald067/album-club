...
function buildSeparatedTokenPattern(token) {
  ...
  return new RegExp(`(?:^|[^a-z0-9])${letters}(?:\\d+)?(?:$|[^a-z0-9])`, "i");
}
...
export function moderateChatPrompt(value) {
  ...
  if (HATE_REQUEST_PATTERNS.some((pattern) => pattern.test(moderationText))) {
    return {
      ok: false,
      value: normalized,
...
export function getChatScopeBoundary(value) {
  ...
  if (!OFF_TOPIC_MEDIA_PATTERNS.some((pattern) => pattern.test(normalized))) {
    return {
      inScope: true,
      value: normalized,
      reason: "",
