...
// All date math uses UTC so server (UTC) and clients in any timezone agree
// on the same album/game for a given aotd key (keys come from toISOString).
/* Every pick below takes an optional `date` and defaults to now. The server
   computes a day's picks for an explicit date (lib/daily-picks.js) and
   scripts/pin-schedule.mjs records them for dates other than today, so none
   of them may read the clock on their own. */
...
export function getListenUrl(album) {
  if (album?.youtubeId) {
    return `https://www.youtube.com/watch?v=${album.youtubeId}`;
  }
  const q = encodeURIComponent(`${album.artist} ${album.title} full album`);
  return `https://www.youtube.com/results?search_query=${q}`;
}

// Index by how many times this game has come up — its "appearance ordinal" —
// rather than by the raw day of year. A game only airs every `cadence` days, so
// indexing on dayOfYear samples the pool at a stride of `cadence`: it visits at
// most 365/cadence slots a year however big the pool is, and collapses to
// pool/cadence slots whenever the two share a factor (a 133-album pool gives 73
// picks a year; 135 gives 27, silently). Walking the ordinal advances exactly
// one step per appearance, so a pool of P runs P appearances before repeating
// and its size can never interact with the cadence.
export function pickRotatingPoolAlbum(
  pool,
  seed,
  { avoidToday = true, cadence = GAME_TYPES.length, date = new Date() } = {},
) {
  if (!Array.isArray(pool) || pool.length === 0) {
    return null;
  }

  const ordinal = Math.floor(getDayOfYear(date) / Math.max(1, cadence));
  const order = seededPermutation(pool.length, seed);
  let idx = order[ordinal % pool.length];

  if (avoidToday && pool.length > 1) {
    const today = getAlbumForDate(date);
    const selected = pool[idx];

    if (selected?.title === today.title && selected?.artist === today.artist) {
      // Jump to the far side of the permutation rather than borrowing the next
      // ordinal's pick — that slot is what the game will land on at its very
      // next airing, so stepping by one turns an avoided collision into the
      // same album twice running.
      idx = order[(ordinal + Math.floor(pool.length / 2)) % pool.length];
    }
  }
...
export function getPuzzleAlbum(date = new Date()) {
  const puzzlePool = ALBUMS.filter((a) => a.recognizable);
  const year = date.getUTCFullYear();
  return pickRotatingPoolAlbum(puzzlePool, year * 31 + 7, { date });
}
...
// Game rotation — 5-day cycle: guess, cover, lyric, heardle, scramble
...
export function getCoverPuzzleAlbum(date = new Date()) {
  const pool = ALBUMS.filter((a) => a.recognizable && a.image);
  const year = date.getUTCFullYear();
  return pickRotatingPoolAlbum(pool, year * 47 + 13, { date });
}

// Artist Scramble — recognizable albums, different seed
export function getScrambleAlbum(date = new Date()) {
  const pool = ALBUMS.filter((a) => a.recognizable);
  const year = date.getUTCFullYear();
  return pickRotatingPoolAlbum(pool, year * 19 + 3, { date });
}

// Heardle — albums with YouTube IDs, different seed
export function getHeardleAlbum(date = new Date()) {
  const pool = ALBUMS.filter((a) => a.recognizable && a.youtubeId);
  if (pool.length === 0) return getCoverPuzzleAlbum(date); // fallback
  const year = date.getUTCFullYear();
  return pickRotatingPoolAlbum(pool, year * 61 + 17, { date });
}

/* Lyric Challenge — recognizable albums that have a lyric entry. The pool
   depends on lib/lyrics.json, which is large, so the caller passes its keys in
   rather than this module importing it (this file ships to the browser).
   Used to live inline in ForumPage; moved here so the server can compute and
   record the day's lyric pick like every other game's. */
export function getLyricAlbum(lyricKeys, date = new Date()) {
  const keys = new Set([...lyricKeys].map((k) => k.toLowerCase()));
  const pool = ALBUMS.filter(
    (album) =>
      album.recognizable &&
      keys.has(`${album.artist} - ${album.title}`.toLowerCase()),
  );
  return pickRotatingPoolAlbum(pool, date.getUTCFullYear() * 37 + 11, {
    date,
  });
}
...
export function scrambleArtist(artist, seed) {
  ...
  // Ensure it's actually different from original. Swapping positions 0 and 1
  // is a no-op whenever those two happen to hold the same character, which
  // handed the puzzle back unscrambled — so swap with the first position that
  // actually differs. When every character is identical no scramble exists.
  if (chars.join("") === artist && chars.length > 1) {
    const differing = chars.findIndex((char) => char !== chars[0]);
    if (differing > 0) {
      [chars[0], chars[differing]] = [chars[differing], chars[0]];
    }
  }
  return chars.join("");
}

// Album vs Album — deterministic daily pairing from full catalog
// Deterministic daily pair from a pool. A fixed *2 stride into a fixed
// even-length permutation repeats the pair every len/2 days, so reshuffle
// per day (seed includes dayOfYear) and take the first two distinct albums
// that aren't today's featured one — full-year variety, same for everyone.
function pickDailyPair(pool, seed, date) {
  const today = getAlbumForDate(date);
  const order = seededPermutation(pool.length, seed + getDayOfYear(date));
  const picks = [];
  for (const idx of order) {
    if (pool[idx].title === today.title) continue;
    picks.push(idx);
    if (picks.length === 2) break;
  }
  return { albumA: pool[picks[0]], albumB: pool[picks[1]] };
}

export function getVersusPair(date = new Date()) {
  return pickDailyPair(ALBUMS, date.getUTCFullYear() * 83 + 23, date);
}

// Blind Taste Test — deterministic daily pairing from albums with youtubeId
export function getTastePair(date = new Date()) {
  const pool = ALBUMS.filter((a) => a.youtubeId);
  return pickDailyPair(pool, date.getUTCFullYear() * 97 + 31, date);
}
...
const MARQUEE_MESSAGES = [
  ...
  (a) =>
    `🎵 Day ${new Date().getUTCDate()} of the month. Album: ${a.title}. Your mission: listen and rate. 🎵`,
  (a) =>
  ...
  () =>
    "🎵 Anonymous on purpose. Rate it, tag the mood, and see whether the room agrees with you. 🎵",
  () =>
  ...
  () =>
    `🔥 ${ALBUMS.length} albums in rotation — canon picks, curveballs, and plenty in between. 🔥`,
  () =>
  ...
  () =>
    "📼 More listeners means stranger averages, stronger arguments, and better vibe data. 📼",
  () =>
  ...
  // More dynamic — today's album
  (a) =>
    `🎬 Soundtrack Corner is open: would ${a.title} score a game, a film, or a TV show? Cast your cue! 🎬`,
  (a) =>
    `🧢 ${a.artist} in ${a.year}: bold move or inevitable classic? The rating box awaits your verdict. 🧢`,
  (a) =>
    `🎧 First time hearing ${a.title}? Lucky you. Some people would pay to get that back. 🎧`,
  (a) =>
    `📻 This is your captain speaking: today's forecast is 100% ${a.genre.toLowerCase()}. 📻`,
  (a) =>
    `🗳️ The room is voting on ${a.title} right now. Silence counts as cowardice. 🗳️`,
  (a) =>
    `🏆 Day ${getDayOfYear()} of the year. Album ${a.year <= 1979 ? "older than most of the internet" : "of the day"}: ${a.title}. 🏆`,
  // More trivia & facts
  () =>
    "📼 The first music video ever played on MTV: 'Video Killed the Radio Star.' 1981. Poetry. 📼",
  () =>
    "💿 Nirvana's 'Nevermind' knocked Michael Jackson off #1 in January 1992. The 90s officially began. 💿",
  () =>
    "🪩 Sgt. Pepper was recorded on a four-track machine. Your phone has more studio than Abbey Road did. 🪩",
  () =>
    "🔮 Wu-Tang made exactly one copy of an album and sold it like a painting. The forum is still arguing. 🔮",
  () =>
    "📼 The groove on one side of a vinyl LP is nearly half a kilometer long. Respect the needle. 📼",
  () =>
    "⚡ The Sony Walkman launched in 1979. Music in your pocket has been ruining commutes' silence ever since. ⚡",
  () =>
    "💿 Fleetwood Mac's 'Rumours' went diamond while the band was actively falling apart. Art, baby. 💿",
  () =>
    "🌙 'Bohemian Rhapsody' runs 5:55 and radio played it anyway. Never let them tell you it's too long. 🌙",
  () =>
    "🎵 A standard CD holds 74 minutes — allegedly sized to fit Beethoven's Ninth Symphony. 🎵",
  // More community & participation
  () =>
    "🕹️ Old forum legends whisper of a code... it starts ↑↑↓↓ and ends in confetti. 🕹️",
  () =>
    "🖱️ That vinyl record on the album card? It spins if you click it. You're welcome. 🖱️",
  () =>
    `⭐ ${ALBUMS.length} albums in the crate, one a day, every one picked on purpose. No filler. ⭐`,
  () =>
    "📅 Streaks are built one day at a time. Yesterday's recap is waiting below if you missed it. 📅",
  () =>
    "🎯 Finished everything today? The Daily Wrap-up has a share card with your name on it. Well, your anonymous name. 🎯",
  () =>
    "👁️ Lurkers welcome. But the rank ladder goes all the way to Legend, just saying. 👁️",
  () =>
    "💌 Wordle-style share buttons everywhere. Copy your results, start an argument somewhere else too. 💌",
  // More hot takes & vibes
  () =>
    "🧊 Track 1 is a promise. Track 2 is the proof. The rest is the argument. 🧊",
  () =>
    "🎹 Skipping the intro track is legal but the forum will know in its heart. 🎹",
  () =>
    "🥇 A perfect album isn't one you can't improve — it's one you wouldn't dare touch. 🥇",
  () =>
    "🌊 Albums are sequenced on purpose. Shuffle is a lifestyle, not a listening strategy. 🌊",
  () =>
    "🫀 If the bridge gives you chills on the fifth listen, that's not a fluke. That's craft. 🫀",
];
