import albumData from "./albums.json";

export const ALBUMS = albumData;

// Seeded PRNG (mulberry32) — deterministic across server and client
function mulberry32(seed) {
  return function () {
    let t = (seed += 0x6d2b79f5);
    t = Math.imul(t ^ (t >>> 15), t | 1);
    t ^= t + Math.imul(t ^ (t >>> 7), t | 61);
    return ((t ^ (t >>> 14)) >>> 0) / 4294967296;
  };
}

// Fisher-Yates shuffle with seeded PRNG — same seed always gives same order
const permutationCache = new Map();

function seededPermutation(length, seed) {
  const cacheKey = `${length}:${seed}`;
  if (permutationCache.has(cacheKey)) return permutationCache.get(cacheKey);
  const rand = mulberry32(seed);
  const indices = Array.from({ length }, (_, i) => i);
  for (let i = indices.length - 1; i > 0; i--) {
    const j = Math.floor(rand() * (i + 1));
    [indices[i], indices[j]] = [indices[j], indices[i]];
  }
  permutationCache.set(cacheKey, indices);
  return indices;
}

// All date math uses UTC so server (UTC) and clients in any timezone agree
// on the same album/game for a given aotd key (keys come from toISOString).
export function getDayOfYear() {
  const now = new Date();
  const start = Date.UTC(now.getUTCFullYear(), 0, 0);
  return Math.floor((now.getTime() - start) / 86400000);
}

export function getTodayAlbum() {
  const year = new Date().getUTCFullYear();
  const dayOfYear = getDayOfYear();
  const order = seededPermutation(ALBUMS.length, year);
  const idx = order[dayOfYear % ALBUMS.length];
  return { ...ALBUMS[idx], key: getTodayKey() };
}

export function getTodayKey() {
  return new Date().toISOString().split("T")[0];
}

export function getListenUrl(album) {
  if (album?.youtubeId) {
    return `https://www.youtube.com/watch?v=${album.youtubeId}`;
  }
  const q = encodeURIComponent(`${album.artist} ${album.title} full album`);
  return `https://www.youtube.com/results?search_query=${q}`;
}

export function pickRotatingPoolAlbum(pool, seed, { avoidToday = true } = {}) {
  if (!Array.isArray(pool) || pool.length === 0) {
    return null;
  }

  const dayOfYear = getDayOfYear();
  const order = seededPermutation(pool.length, seed);
  let idx = order[dayOfYear % pool.length];

  if (avoidToday && pool.length > 1) {
    const today = getTodayAlbum();
    const selected = pool[idx];

    if (selected?.title === today.title && selected?.artist === today.artist) {
      idx = order[(dayOfYear + 1) % pool.length];
    }
  }

  return pool[idx];
}

export function getDateString() {
  return new Date().toLocaleDateString("en-US", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
    timeZone: "UTC",
  });
}

// Puzzle: uses recognizable albums only, different seed so it differs from today's featured
export function getPuzzleAlbum() {
  const puzzlePool = ALBUMS.filter((a) => a.recognizable);
  const year = new Date().getUTCFullYear();
  return pickRotatingPoolAlbum(puzzlePool, year * 31 + 7);
}

export function getPuzzleKey() {
  return `puzzle-${getTodayKey()}`;
}

export function getPuzzleClues(album) {
  return [
    { label: "Genre", value: album.genre },
    { label: "Decade", value: `${Math.floor(album.year / 10) * 10}s` },
    { label: "Words in title", value: `${album.title.split(/\s+/).length}` },
    { label: "Artist starts with", value: `"${album.artist[0]}"` },
    { label: "Release year", value: `${album.year}` },
    { label: "Artist", value: album.artist },
  ];
}

// Get album for any date (used by Archive)
export function getAlbumForDate(date) {
  const year = date.getUTCFullYear();
  const start = Date.UTC(year, 0, 0);
  const dayOfYear = Math.floor((date.getTime() - start) / 86400000);
  const order = seededPermutation(ALBUMS.length, year);
  const idx = order[dayOfYear % ALBUMS.length];
  return { ...ALBUMS[idx], key: date.toISOString().split("T")[0] };
}

// Game rotation — 5-day cycle: guess, cover, lyric, heardle, scramble
const GAME_TYPES = ["guess", "cover", "lyric", "heardle", "scramble"];

export function getGameType(date) {
  const d = date || new Date();
  const start = Date.UTC(d.getUTCFullYear(), 0, 0);
  const dayOfYear = Math.floor((d.getTime() - start) / 86400000);
  return GAME_TYPES[dayOfYear % GAME_TYPES.length];
}

// Cover Art Challenge — uses albums with cover art, different seed
export function getCoverPuzzleAlbum() {
  const pool = ALBUMS.filter((a) => a.recognizable && a.image);
  const year = new Date().getUTCFullYear();
  return pickRotatingPoolAlbum(pool, year * 47 + 13);
}

// Artist Scramble — recognizable albums, different seed
export function getScrambleAlbum() {
  const pool = ALBUMS.filter((a) => a.recognizable);
  const year = new Date().getUTCFullYear();
  return pickRotatingPoolAlbum(pool, year * 19 + 3);
}

// Heardle — albums with YouTube IDs, different seed
export function getHeardleAlbum() {
  const pool = ALBUMS.filter((a) => a.recognizable && a.youtubeId);
  if (pool.length === 0) return getCoverPuzzleAlbum(); // fallback
  const year = new Date().getUTCFullYear();
  return pickRotatingPoolAlbum(pool, year * 61 + 17);
}

// Lyric puzzle album is picked inline in ForumPage (seed year * 37 + 11)
// because the pool depends on which albums have usable lyric data.

// Scramble utility — deterministic scramble of artist name
export function scrambleArtist(artist, seed) {
  const chars = artist.split("");
  const rand = mulberry32(seed);
  for (let i = chars.length - 1; i > 0; i--) {
    const j = Math.floor(rand() * (i + 1));
    [chars[i], chars[j]] = [chars[j], chars[i]];
  }
  // Ensure it's actually different from original
  if (chars.join("") === artist && chars.length > 1) {
    [chars[0], chars[1]] = [chars[1], chars[0]];
  }
  return chars.join("");
}

// ─── Genre Bingo ───

/** Map every album genre to a broad bingo category */
const GENRE_TO_CATEGORY = {};
const BINGO_CATEGORIES_MAP = {
  Rock: [
    "Rock",
    "Alternative Rock",
    "Indie Rock",
    "Art Rock",
    "Experimental Rock",
    "Garage Rock",
    "Glam Rock",
    "Grunge",
    "Noise Rock",
    "Soft Rock",
    "Soft Rock/Indie",
    "Britpop",
    "Madchester",
    "Rock/Anime",
    "Rock/Live",
  ],
  "Hip-Hop": [
    "Hip-Hop",
    "Abstract Hip-Hop",
    "Conscious Hip-Hop",
    "Experimental Hip-Hop",
    "Underground Hip-Hop",
    "Southern Hip-Hop",
    "West Coast Hip-Hop",
    "Hip-Hop/West Coast",
    "Hip-Hop/Anime",
    "Hip-Hop/Electronic",
    "Hip-Hop/Experimental",
    "Hip-Hop/Funk",
    "Hip-Hop/Jazz",
    "Hip-Hop/Jazz Rap",
    "Hip-Hop/Soul",
    "Alternative/Hip-Hop",
    "Instrumental Hip-Hop",
    "Jazz Hip-Hop",
    "Jazz Rap",
  ],
  Electronic: [
    "Electronic",
    "Electronic/Alt-Pop",
    "Electronic/Anime",
    "Electronic/Art Pop",
    "Electronic/Dance",
    "Electronic/Disco",
    "Electronic/DJ Set",
    "Electronic/Downtempo",
    "Electronic/Experimental",
    "Electronic/Experimental Pop",
    "Electronic/Folktronica",
    "Electronic/House",
    "Electronic/Jazz",
    "Electronic/Moog",
    "Electronic/R&B",
    "Electronic/Rock",
    "Electronic/Soundtrack",
    "Electropop",
    "Electropop/French House",
    "Colombian Electronic",
    "Dark Electronic",
    "IDM",
    "IDM/Ambient",
    "Afrobeat/Electronic",
  ],
  Jazz: [
    "Jazz",
    "Jazz Fusion",
    "Jazz-Funk",
    "Jazz/Ambient",
    "Jazz/Anime",
    "Jazz/Improvisation",
    "Jazz/Mix",
  ],
  Folk: [
    "Folk",
    "Folk/Americana",
    "Folk/Anime",
    "Art Folk",
    "Chamber Folk",
    "Freak Folk",
    "Experimental Folk",
    "Indie Folk",
  ],
  Pop: [
    "Pop",
    "Pop Rock",
    "Pop/Hip-Hop",
    "Pop/Nu-Metal",
    "Pop/R&B",
    "Pop/Rap",
    "Art Pop",
    "Art Pop/Electronic",
    "Art Pop/Folk",
    "Art Pop/Rock",
    "Experimental Pop",
    "Indie Pop",
    "Indie Pop/Spoken Word",
    "Psychedelic Pop",
    "Disco Pop",
    "Country Pop",
  ],
  "R&B / Soul": [
    "R&B",
    "R&B/Art Pop",
    "R&B/Electronic",
    "R&B/Funk",
    "R&B/Gospel",
    "R&B/Mix",
    "R&B/Neo-Soul",
    "R&B/Pop",
    "Dark R&B",
    "Soul",
    "Soul/Funk",
    "Soul/Live",
    "Brazilian Soul",
    "Country/Soul",
    "K-R&B/Mix",
    "Rap/Neo-Soul",
  ],
  Metal: [
    "Heavy Metal",
    "Progressive Metal",
    "Progressive Metal/Live",
    "Progressive Death Metal",
    "Metalcore",
    "Blackgaze",
  ],
  Punk: [
    "Punk Rock",
    "Post-Punk",
    "Post-Punk Revival",
    "Post-Punk/Shoegaze",
    "Dance-Punk",
    "Hardcore/Pop-Punk",
  ],
  Ambient: [
    "Ambient",
    "Ambient/Anime",
    "Ambient/Electronic",
    "Ambient/Focus",
    "Ambient/Hauntology",
    "Ambient/Meditation",
    "Ambient/Mix",
    "Ambient/Nature",
    "Ambient/Soundtrack",
    "Ambient/Vaporwave",
    "Dubstep/Ambient",
  ],
  "Synth / New Wave": [
    "Synth-Pop",
    "Synth-Pop/R&B",
    "Synth/Anime",
    "Synth/Soundtrack",
    "Synthwave/Mix",
    "Synthwave/Soundtrack",
    "New Wave",
    "New Wave/Argentine Rock",
    "New Wave/Live",
  ],
  "Prog Rock": [
    "Progressive Rock",
    "Progressive Rock/Live",
    "Post-Rock",
    "Post-Rock/Chamber Pop",
  ],
  "Psych Rock": [
    "Psychedelic Rock",
    "Psychedelic/Live",
    "Acid House/Rock",
    "Country/Psychedelic",
  ],
  Shoegaze: ["Shoegaze", "Shoegaze/Indie Pop", "Dream Pop", "Dream Pop/Dub"],
  "Trip-Hop": [
    "Trip-Hop",
    "Trip-Hop/Instrumental",
    "Chillwave",
    "Chillwave/Ambient",
    "Chillout/Mix",
  ],
  "House / Techno": [
    "House/Techno",
    "House/DJ Set",
    "Dance/House",
    "Techno/DJ Set",
    "French House",
    "French House/Mix",
    "Deep House/Mix",
  ],
  Lofi: [
    "Lofi",
    "Lofi Hip-Hop",
    "Lofi Jazz",
    "Lofi/Anime",
    "Lofi/Chill",
    "Lofi/Chillhop",
    "Lofi/Experimental",
    "Lofi/Indie",
    "Lofi/R&B",
  ],
  Country: [
    "Alt-Country",
    "Country/Indie",
    "Americana",
    "Roots/Americana",
    "Folk/Americana",
  ],
  "K-Pop": [
    "K-Pop",
    "K-Pop/Electropop",
    "K-Shibuya-kei",
    "City Pop",
    "City Pop/Mix",
    "Shibuya-kei/Pop",
  ],
  Afrobeat: ["Afrobeat", "Desert Blues", "Desert Rock/Tuareg"],
  World: [
    "Latin/World",
    "Cuban/Son",
    "MPB",
    "Tropiália",
    "Indian Classical",
    "Indian Fusion",
  ],
  "Rap / Experimental": [
    "Rap/Experimental",
    "Hyperpop",
    "Hyperpop/Dance",
    "Hyperpop/Experimental",
  ],
  Soundtrack: [
    "Orchestral/Anime",
    "Orchestral/Soundtrack",
    "Cinematic/Anime",
    "Classical/Piano",
    "Minimalist",
  ],
  "Avant-Garde": [
    "Avant-Garde/Anime",
    "Plunderphonics",
    "Witch House/Experimental",
    "Vaporwave",
    "Future Funk",
  ],
  "Indie / Other": [
    "Indie/Electronic",
    "Indie/Mix",
    "Acoustic/Live",
    "Various/Live",
    "Gothic Rock",
  ],
};

// Build reverse lookup
const BINGO_CATEGORIES = Object.keys(BINGO_CATEGORIES_MAP);
for (const [cat, genres] of Object.entries(BINGO_CATEGORIES_MAP)) {
  for (const g of genres) GENRE_TO_CATEGORY[g] = cat;
}

/** Get the bingo category for a genre string */
export function getGenreCategory(genre) {
  return GENRE_TO_CATEGORY[genre] || "Indie / Other";
}

/** Generate a deterministic 5x5 bingo card for a given month */
export function getBingoCard(year, month) {
  const seed = year * 100 + month;
  const order = seededPermutation(BINGO_CATEGORIES.length, seed * 73 + 29);
  const cells = [];
  for (let i = 0; i < 25; i++) {
    if (i === 12) {
      cells.push({ category: "FREE", free: true });
    } else {
      const ci = i < 12 ? i : i - 1;
      cells.push({
        category: BINGO_CATEGORIES[order[ci % BINGO_CATEGORIES.length]],
        free: false,
      });
    }
  }
  return cells;
}

/** Get matched genre categories for a month (from day 1 to current day) */
export function getMonthMatches(year, month) {
  const matched = new Set();
  const now = new Date();
  const lastDay =
    year === now.getUTCFullYear() && month === now.getUTCMonth() + 1
      ? now.getUTCDate()
      : new Date(Date.UTC(year, month, 0)).getUTCDate();
  for (let d = 1; d <= lastDay; d++) {
    const album = getAlbumForDate(new Date(Date.UTC(year, month - 1, d)));
    matched.add(getGenreCategory(album.genre));
  }
  return matched;
}

/** Check for bingo (5 in a row/col/diag) */
export function checkBingo(card, matched) {
  const m = card.map((c) => c.free || matched.has(c.category));
  // rows
  for (let r = 0; r < 5; r++) {
    if (m.slice(r * 5, r * 5 + 5).every(Boolean)) return true;
  }
  // cols
  for (let c = 0; c < 5; c++) {
    if ([0, 1, 2, 3, 4].every((r) => m[r * 5 + c])) return true;
  }
  // diags
  if ([0, 6, 12, 18, 24].every((i) => m[i])) return true;
  if ([4, 8, 12, 16, 20].every((i) => m[i])) return true;
  return false;
}

/** Find lines that are 1 away from bingo — returns array of { type, index, missing } */
export function getNearBingoLines(card, matched) {
  const m = card.map((c) => c.free || matched.has(c.category));
  const lines = [];
  // rows
  for (let r = 0; r < 5; r++) {
    const indices = [0, 1, 2, 3, 4].map((c) => r * 5 + c);
    const count = indices.filter((i) => m[i]).length;
    if (count === 4) {
      const missingIdx = indices.find((i) => !m[i]);
      lines.push({
        type: "row",
        index: r,
        missing: card[missingIdx].category,
        cells: indices,
      });
    }
  }
  // cols
  for (let c = 0; c < 5; c++) {
    const indices = [0, 1, 2, 3, 4].map((r) => r * 5 + c);
    const count = indices.filter((i) => m[i]).length;
    if (count === 4) {
      const missingIdx = indices.find((i) => !m[i]);
      lines.push({
        type: "col",
        index: c,
        missing: card[missingIdx].category,
        cells: indices,
      });
    }
  }
  // diags
  const diag1 = [0, 6, 12, 18, 24];
  if (diag1.filter((i) => m[i]).length === 4) {
    const missingIdx = diag1.find((i) => !m[i]);
    lines.push({
      type: "diag",
      index: 0,
      missing: card[missingIdx].category,
      cells: diag1,
    });
  }
  const diag2 = [4, 8, 12, 16, 20];
  if (diag2.filter((i) => m[i]).length === 4) {
    const missingIdx = diag2.find((i) => !m[i]);
    lines.push({
      type: "diag",
      index: 1,
      missing: card[missingIdx].category,
      cells: diag2,
    });
  }
  return lines;
}

// Album vs Album — deterministic daily pairing from full catalog
export function getVersusPair() {
  const year = new Date().getUTCFullYear();
  const dayOfYear = getDayOfYear();
  const today = getTodayAlbum();
  const order = seededPermutation(ALBUMS.length, year * 83 + 23);
  let iA = order[(dayOfYear * 2) % ALBUMS.length];
  let iB = order[(dayOfYear * 2 + 1) % ALBUMS.length];
  if (ALBUMS[iA].title === today.title)
    iA = order[(dayOfYear * 2 + 2) % ALBUMS.length];
  if (ALBUMS[iB].title === today.title || iB === iA)
    iB = order[(dayOfYear * 2 + 3) % ALBUMS.length];
  return { albumA: ALBUMS[iA], albumB: ALBUMS[iB] };
}

// Blind Taste Test — deterministic daily pairing from albums with youtubeId
export function getTastePair() {
  const pool = ALBUMS.filter((a) => a.youtubeId);
  const year = new Date().getUTCFullYear();
  const dayOfYear = getDayOfYear();
  const today = getTodayAlbum();
  const order = seededPermutation(pool.length, year * 97 + 31);
  let iA = order[(dayOfYear * 2) % pool.length];
  let iB = order[(dayOfYear * 2 + 1) % pool.length];
  if (pool[iA].title === today.title)
    iA = order[(dayOfYear * 2 + 2) % pool.length];
  if (pool[iB].title === today.title || iB === iA)
    iB = order[(dayOfYear * 2 + 3) % pool.length];
  return { albumA: pool[iA], albumB: pool[iB] };
}

// Rotating marquee messages — one per day, some dynamic with album info
const MARQUEE_MESSAGES = [
  // Dynamic — today's album
  (a) =>
    `🎵 Today's pick: ${a.genre} from ${a.year}. What's your take? Rate it, vibe it, guess it! 🎵`,
  (a) =>
    `✨ ${a.artist} dropped this in ${a.year}. Were you there? Rate below! ✨`,
  (a) =>
    `🌙 Spinning ${a.genre.toLowerCase()} tonight. ${a.title} by ${a.artist}. How does it hit? 🌙`,
  (a) =>
    `🔮 Today's genre: ${a.genre}. Can you feel the vibe? Cast your mood below! 🔮`,
  (a) => `🌪️ Genre roulette landed on ${a.genre} today. Embrace the chaos! 🌪️`,
  (a) =>
    `🎵 Day ${new Date().getUTCDate()} of the month. Album: ${a.title}. Your mission: listen and rate. 🎵`,
  (a) =>
    `💿 ${a.title} — ${a.genre}, ${a.year}. One album. One day. A thousand opinions. 💿`,
  (a) =>
    `🪩 ${a.artist} is on deck. ${a.genre} vibes all day. What's your rating? 🪩`,
  (a) =>
    `📼 Throwback alert: ${a.year} gave us ${a.title}. Does it still hold up? You decide. 📼`,
  (a) =>
    `🔥 Today's challenge: listen to ${a.title} front to back and rate it honestly. No skipping! 🔥`,
  (a) =>
    `🌿 ${a.genre} day. Let ${a.artist} set the mood. Rate and vibe when you're ready. 🌿`,
  (a) =>
    `⚡ Can you guess today's puzzle album? Hint: it's NOT ${a.title}. Good luck! ⚡`,
  // Music trivia & facts
  () =>
    "📼 Did you know? The average album takes 6-12 months to record. Some legends did it in days. 📼",
  () =>
    "🪩 Fun fact: The first album ever sold on CD was Billy Joel's '52nd Street' in 1982. 🪩",
  () =>
    "📼 The vinyl revival is real — over 43 million records sold last year. Long live physical media. 📼",
  () =>
    "🔮 The longest album ever recorded is over 13 hours. We promise today's pick is shorter. 🔮",
  () =>
    "💿 The Beatles recorded their debut album in a single day — 585 minutes of studio time. 💿",
  () =>
    "🎵 The term 'album' comes from the Latin 'albus' (white) — for the blank covers of 78rpm sets. 🎵",
  () =>
    "📼 The first gold record was awarded in 1958. It was Perry Como's 'Catch a Falling Star.' 📼",
  () =>
    "🪩 Fela Kuti's longest song is 45 minutes. One track, one side of a vinyl. Respect. 🪩",
  () =>
    "💿 The 8-track tape was invented in 1964. By 1982 it was dead. Pour one out. 💿",
  () =>
    "🔥 The loudest album ever measured hit 137.7 dB. Your ears are thankful for headphone limits. 🔥",
  () =>
    "📼 Bob Dylan's 'Like a Rolling Stone' was rejected by his own label before becoming a classic. 📼",
  () =>
    "🌙 Brian Eno coined 'ambient music' in 1978. He made it while bedridden listening to rain. 🌙",
  () =>
    "⚡ Kraftwerk built their own instruments. The future of electronic music started in a Dusseldorf studio. ⚡",
  () =>
    "🪩 Studio 54 opened in 1977. Disco was dead by 1981. Four years that changed nightlife forever. 🪩",
  () =>
    "💿 Miles Davis recorded 'Kind of Blue' with almost no rehearsal. Most takes were first takes. 💿",
  () =>
    "📼 Motown's studio was so small the musicians called it 'the Snakepit.' Hits came out anyway. 📼",
  // Community & participation
  () =>
    "🎵 Anonymous on purpose. Rate it, tag the mood, and see whether the room agrees with you. 🎵",
  () =>
    "💿 Welcome to the club! New album every day at midnight UTC. Don't miss it. 💿",
  () =>
    "⚡ Tip: The puzzle uses a different album than today's feature. Two chances to discover! ⚡",
  () =>
    `🔥 ${ALBUMS.length} albums in rotation — canon picks, curveballs, and plenty in between. 🔥`,
  () =>
    "🪩 Groove check: if today's album makes you move, smash that Groovy vibe button. 🪩",
  () =>
    "🌿 You can pick up to 3 vibes per album. Choose wisely — or chaotically. Your call. 🌿",
  () =>
    "✨ Your rating is anonymous. Be honest. Be brutal. Be kind. Whatever feels right. ✨",
  () =>
    "💢 Disagree with the average rating? Good. That means the club is working. 💢",
  () =>
    "🔮 The puzzle gives you 6 guesses. Most people crack it by clue 3. Can you beat the average? 🔮",
  () =>
    "📼 More listeners means stranger averages, stronger arguments, and better vibe data. 📼",
  () =>
    "🌙 Late night listening hits different. If you're here past midnight — respect. 🌙",
  // Hot takes & vibes
  () =>
    "🥀 Hot take: your 6/10 is someone else's 10/10. That's the beauty of music. 🥀",
  () =>
    "💢 Controversial opinion incoming: the best albums are the ones that divide the room. 💢",
  () =>
    "🔮 Mind-bending albums exist. If today's one of them, you know what vibe to pick. 🔮",
  () =>
    "💔 Some albums break your heart in the best way. Rate honestly — we won't judge. 💔",
  () =>
    "✨ Every album deserves at least one full listen. Even the weird ones. Especially the weird ones. ✨",
  () =>
    "🌿 Slow down. Put on headphones. Listen to the whole thing front to back. You deserve it. 🌿",
  () =>
    "🥀 Not every album is a 10. Not every album is a 1. The magic lives in the middle. 🥀",
  () =>
    "🔥 If an album makes you feel something — anything — it did its job. Rate accordingly. 🔥",
  () => "💔 The saddest albums are often the most beautiful. Lean into it. 💔",
  () =>
    "🌪️ Some albums are controlled chaos. If today's one of those, the Chaotic vibe is right there. 🌪️",
  () =>
    "⚡ An album that makes you restart it immediately? That's a 10. You know the rules. ⚡",
  () => "🪩 Remember: there are no wrong vibes. Only honest ones. 🪩",
  // More dynamic — today's album
  (a) =>
    `🎬 Soundtrack Corner is open: would ${a.title} score a game, a film, or a TV show? Cast your cue! 🎬`,
  (a) =>
    `🧢 ${a.artist} in ${a.year}: bold move or inevitable classic? The rating box awaits your verdict. 🧢`,
  (a) =>
    `🎧 First time hearing ${a.title}? Lucky you. Some people would pay to get that back. 🎧`,
  (a) =>
    `📻 This is your captain speaking: today's forecast is 100% ${a.genre.toLowerCase()}. 📻`,
  (a) =>
    `🗳️ The room is voting on ${a.title} right now. Silence counts as cowardice. 🗳️`,
  (a) =>
    `🏆 Day ${getDayOfYear()} of the year. Album ${a.year <= 1979 ? "older than most of the internet" : "of the day"}: ${a.title}. 🏆`,
  // More trivia & facts
  () =>
    "📼 The first music video ever played on MTV: 'Video Killed the Radio Star.' 1981. Poetry. 📼",
  () =>
    "💿 Nirvana's 'Nevermind' knocked Michael Jackson off #1 in January 1992. The 90s officially began. 💿",
  () =>
    "🪩 Sgt. Pepper was recorded on a four-track machine. Your phone has more studio than Abbey Road did. 🪩",
  () =>
    "🔮 Wu-Tang made exactly one copy of an album and sold it like a painting. The forum is still arguing. 🔮",
  () =>
    "📼 The groove on one side of a vinyl LP is nearly half a kilometer long. Respect the needle. 📼",
  () =>
    "⚡ The Sony Walkman launched in 1979. Music in your pocket has been ruining commutes' silence ever since. ⚡",
  () =>
    "💿 Fleetwood Mac's 'Rumours' went diamond while the band was actively falling apart. Art, baby. 💿",
  () =>
    "🌙 'Bohemian Rhapsody' runs 5:55 and radio played it anyway. Never let them tell you it's too long. 🌙",
  () =>
    "🎵 A standard CD holds 74 minutes — allegedly sized to fit Beethoven's Ninth Symphony. 🎵",
  // More community & participation
  () =>
    "🕹️ Old forum legends whisper of a code... it starts ↑↑↓↓ and ends in confetti. 🕹️",
  () =>
    "🖱️ That vinyl record on the album card? It spins if you click it. You're welcome. 🖱️",
  () =>
    "⭐ Genre Bingo fills in as the month rolls. Five in a row and you've earned bragging rights. ⭐",
  () =>
    "📅 Streaks are built one day at a time. Yesterday's recap is waiting below if you missed it. 📅",
  () =>
    "🎯 Finished everything today? The Daily Wrap-up has a share card with your name on it. Well, your anonymous name. 🎯",
  () =>
    "👁️ Lurkers welcome. But the rank ladder goes all the way to Legend, just saying. 👁️",
  () =>
    "💌 Wordle-style share buttons everywhere. Copy your results, start an argument somewhere else too. 💌",
  // More hot takes & vibes
  () =>
    "🧊 Track 1 is a promise. Track 2 is the proof. The rest is the argument. 🧊",
  () =>
    "🎹 Skipping the intro track is legal but the forum will know in its heart. 🎹",
  () =>
    "🥇 A perfect album isn't one you can't improve — it's one you wouldn't dare touch. 🥇",
  () =>
    "🌊 Albums are sequenced on purpose. Shuffle is a lifestyle, not a listening strategy. 🌊",
  () =>
    "🫀 If the bridge gives you chills on the fifth listen, that's not a fluke. That's craft. 🫀",
];

export function getMarqueeMessage(album) {
  const dayOfYear = getDayOfYear();
  const idx = dayOfYear % MARQUEE_MESSAGES.length;
  return MARQUEE_MESSAGES[idx](album);
}

export const VIBES = [
  {
    emoji: "🥀",
    label: "Melancholy",
    icon: "/pixel-icons/weather-cresent-moon-stars.svg",
    color: "#d0c4df",
  },
  {
    emoji: "⚡",
    label: "Energetic",
    icon: "/pixel-icons/interface-essential-flash.svg",
    color: "#ffe0a3",
  },
  {
    emoji: "📼",
    label: "Nostalgic",
    icon: "/pixel-icons/music-walkman-cassette.svg",
    color: "#e8d5b8",
  },
  {
    emoji: "🌪️",
    label: "Chaotic",
    icon: "/pixel-icons/entertainment-events-hobbies-bomb.svg",
    color: "#f5c4b8",
  },
  {
    emoji: "🌿",
    label: "Chill",
    icon: "/pixel-icons/ecology-growth-plant.svg",
    color: "#c4e0c8",
  },
  {
    emoji: "🔥",
    label: "Intense",
    icon: "/pixel-icons/social-rewards-trends-hot-flame.svg",
    color: "#e8b8b8",
  },
  {
    emoji: "🪩",
    label: "Groovy",
    icon: "/pixel-icons/music-vinyl-record.svg",
    color: "#f0d4e8",
  },
  {
    emoji: "🌙",
    label: "Dreamy",
    icon: "/pixel-icons/weather-moon.svg",
    color: "#c4d4e8",
  },
  {
    emoji: "💢",
    label: "Angry",
    icon: "/pixel-icons/interface-essential-skull-1.svg",
    color: "#e0b0a0",
  },
  {
    emoji: "💔",
    label: "Heartbreaking",
    icon: "/pixel-icons/romance-heart-lock.svg",
    color: "#d4b8c8",
  },
  {
    emoji: "🔮",
    label: "Mind-bending",
    icon: "/pixel-icons/design-magic-wand.svg",
    color: "#c8c4e0",
  },
  {
    emoji: "✨",
    label: "Uplifting",
    icon: "/pixel-icons/social-rewards-rating-star-1.svg",
    color: "#f0e8a8",
  },
];

// Pixel icons for carousel variety (Streamline Pixel, CC BY 4.0)
export const CAROUSEL_ICONS = [
  { src: "/pixel-icons/music-headphones-human.svg", title: "Headphones" },
  { src: "/pixel-icons/music-vinyl-record.svg", title: "Vinyl Record" },
  { src: "/pixel-icons/music-radio-stereo.svg", title: "Radio" },
  { src: "/pixel-icons/music-notes-music-1.svg", title: "Music Notes" },
  { src: "/pixel-icons/music-speaker.svg", title: "Speaker" },
  { src: "/pixel-icons/music-microphone-1.svg", title: "Microphone" },
  {
    src: "/pixel-icons/music-album-cd-disk-playlist.svg",
    title: "CD Playlist",
  },
  { src: "/pixel-icons/music-disk-cd-1.svg", title: "CD" },
  { src: "/pixel-icons/music-walkman-cassette.svg", title: "Walkman" },
  { src: "/pixel-icons/music-notes-music-2.svg", title: "Notes" },
  {
    src: "/pixel-icons/entertainment-events-hobbies-record-player.svg",
    title: "Record Player",
  },
  {
    src: "/pixel-icons/entertainment-events-hobbies-game-machines-arcade-1.svg",
    title: "Arcade",
  },
  {
    src: "/pixel-icons/entertainment-events-hobbies-horror-ghost.svg",
    title: "Ghost",
  },
  {
    src: "/pixel-icons/entertainment-events-hobbies-popcorn.svg",
    title: "Popcorn",
  },
  {
    src: "/pixel-icons/entertainment-events-hobbies-ticket.svg",
    title: "Ticket",
  },
  {
    src: "/pixel-icons/entertainment-events-hobbies-glasses-3d.svg",
    title: "3D Glasses",
  },
  {
    src: "/pixel-icons/entertainment-events-hobbies-board-game-dice.svg",
    title: "Dice",
  },
  { src: "/pixel-icons/food-drink-coffee-cup.svg", title: "Coffee" },
  { src: "/pixel-icons/food-drink-fruit-cherry.svg", title: "Cherry" },
  { src: "/pixel-icons/food-drink-desert-icecream.svg", title: "Ice Cream" },
  { src: "/pixel-icons/food-drink-pizza.svg", title: "Pizza" },
  { src: "/pixel-icons/pet-animals-cat.svg", title: "Cat" },
  { src: "/pixel-icons/pet-animals-frog-face.svg", title: "Frog" },
  { src: "/pixel-icons/pet-animals-rabbit-1.svg", title: "Rabbit" },
  { src: "/pixel-icons/weather-rainbow.svg", title: "Rainbow" },
  { src: "/pixel-icons/weather-meteor.svg", title: "Meteor" },
  {
    src: "/pixel-icons/design-color-painting-palette.svg",
    title: "Palette",
  },
  { src: "/pixel-icons/design-color-spray.svg", title: "Spray Paint" },
  {
    src: "/pixel-icons/interface-essential-heart-favorite.svg",
    title: "Heart",
  },
  { src: "/pixel-icons/interface-essential-trophy.svg", title: "Trophy" },
  { src: "/pixel-icons/interface-essential-crown.svg", title: "Crown" },
  {
    src: "/pixel-icons/interface-essential-pacman-loading.svg",
    title: "Pac-Man",
  },
  {
    src: "/pixel-icons/interface-essential-light-bulb.svg",
    title: "Lightbulb",
  },
  {
    src: "/pixel-icons/social-rewards-trends-hot-flame.svg",
    title: "Flame",
  },
  { src: "/pixel-icons/social-rewards-vip-crown-king.svg", title: "King" },
  { src: "/pixel-icons/social-rewards-reward-gift.svg", title: "Gift" },
  { src: "/pixel-icons/school-science-test-flask.svg", title: "Flask" },
  {
    src: "/pixel-icons/computers-devices-electronics-vintage-mac.svg",
    title: "Vintage Mac",
  },
  {
    src: "/pixel-icons/computers-devices-electronics-television-vintage.svg",
    title: "Vintage TV",
  },
  {
    src: "/pixel-icons/computers-devices-electronics-tape-cassette.svg",
    title: "Cassette Tape",
  },
  {
    src: "/pixel-icons/entertainment-events-hobbies-game-machines-arcade-2.svg",
    title: "Arcade Machine",
  },
];
