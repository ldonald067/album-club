...
function pickDailyPair(pool, seed) {
  const today = getTodayAlbum();
  const order = seededPermutation(pool.length, seed + getDayOfYear());
  const picks = [];
...
export function getVersusPair() {
  const year = new Date().getUTCFullYear();
  return pickDailyPair(ALBUMS, year * 83 + 23);
}
...
export function getTastePair() {
  ...
  const year = new Date().getUTCFullYear();
  return pickDailyPair(pool, year * 97 + 31);
}
