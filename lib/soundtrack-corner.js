...
/* The decade scene-note used to be pasted onto all three pitches, so every
   album without a curated override closed its game, film and TV cards with the
   same sentence, three times in one view. Hand it to a single card, rotating by
   seed so it is not always the same medium that carries it. */
const SCENE_NOTE_MEDIA = ["game", "film", "tv"];
...
/* Keyed off the album, never the card: each card's own seed is salted by medium
   ...
   themselves independently and the sentence still landed 2-3 times on about a
   quarter of albums. One album, one carrier. */
function sceneNoteFor(medium, decadeFlavor, album) {
  const carrier =
    SCENE_NOTE_MEDIA[
      getAlbumSeed(album, "scene-note") % SCENE_NOTE_MEDIA.length
    ];
  return carrier === medium ? ` ${decadeFlavor.sceneNote}` : "";
}
...
const MEDIA_CARD_CONFIG = {
  game: {
    ...
    buildBody(album, profile, decadeFlavor, seed) {
      return `If ${album.title} scored ${pick(profile.gameLocations, seed, 2)}, it would be the part where ${pick(profile.gameActions, seed, 3)}. ${pick(GAME_CODAS, seed, 4)}${sceneNoteFor("game", decadeFlavor, album)}`;
    },
  },
  film: {
    label: "Film",
    ...
    buildBody(album, profile, decadeFlavor, seed) {
      return `${album.title} works for ${pick(profile.filmShots, seed, 2)}, especially when ${pick(profile.filmTurns, seed, 3)}. ${pick(FILM_CODAS, seed, 4)}${sceneNoteFor("film", decadeFlavor, album)}`;
    },
  },
  tv: {
    label: "TV",
    ...
    titleField: "tvTitles",
    buildBody(album, profile, decadeFlavor, seed) {
      return `Use it for ${pick(profile.tvSetups, seed, 2)}, when ${pick(profile.tvTurns, seed, 3)}. ${pick(TV_CODAS, seed, 4)}${sceneNoteFor("tv", decadeFlavor, album)}`;
    },
  },
};
...
const RECOMMENDATION_PHRASINGS = {
  sameArtist: [
    ...
    ({ trait }) => `Another room in the same house, and the ${trait} came with it.`,
  ],
  sharedRecent: [
    ({ segment, trait }) =>
...
export function buildSoundtrackCorner(album) {
  ...
  return {
    ...
    bridgeNote:
      override?.bridgeNote ||
      `${album.genre} from the ${decadeLabel} lands best on screen when ${profile.bridgeFocus} and ${profile.texture} stay louder than generic background mood.`,
