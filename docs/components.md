...
Its own nav tab (formerly the Chat Booth — the AI chat was removed in July 2026). Renders today's album as game / film / TV cue music: album-specific scene cards, two rotating extra angles (for example boss-fight energy or best-fit game studio), a short "listen for" list, and a clickable "listen next" recommendation row with one-line reasons plus YouTube links for today's album and each follow-up pick. Loaded with `next/dynamic` so its weight stays off the main chunk — but **server-rendered**, and the `ssr: false` it used to carry is gone. That flag was right while the corner was a hidden tab of the one home page and wrong the moment `/soundtrack` became a route whose purpose is being findable: a crawler was served `Opening Soundtrack Corner...` and nothing else. Removing it costs the home page nothing (measured on a production build: 13 JS requests either side, 215KB → 212KB) because the corner only renders when its own tab is showing, so Home never pulls it regardless — the boundary that keeps the weight out is the dynamic import, not the flag. `readCuePicks` gained a storage guard to match, since it is the one place the component touches `localStorage` during render rather than in an effect.

The corner arrives through a Suspense boundary, so the initial HTML carries the fallback first and the real markup streams in after it. Anything that executes JavaScript — including Google — gets the full panel; a client that renders no JavaScript at all sees the content present in the document but not swapped in. That is a large improvement on the content being absent entirely, and not the same thing as a fully static render.
...
The Archive's **cue log** reads the same two records back: a `Room` column fed by `GET /api/soundtrack/history` (a 30-day window of `soundtrack_votes`, matching the 30 days the table already lists) and a `You` column from localStorage, hidden at mobile like genre and year. **Each half appears only once it has something to say (2026-08-21).** At this site's actual traffic a day rarely reaches the two votes the room floor needs, and a `Room` column showing "·" thirty rows deep reads as a broken feature rather than a quiet one. `showRoom` and `showMine` are computed after the history request settles, so the table does not shuffle its own columns mid-load, and with neither populated the Archive is exactly the table it was before. It shows words, not the vote's emoji — "🎮🎬" in an 11px table is a cipher with its meaning hiding in a title attribute. Its padding rule has to be written `.archive-table th.archive-cue-head`: a bare class loses to `.archive-table th` at (0,1,1), and without it the table overflows 375px by a pixel.
