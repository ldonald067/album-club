...
Which album a game picks comes from `pickRotatingPoolAlbum`, which indexes by **appearance ordinal**, not `dayOfYear` — see `docs/album-data.md`. Pool sizes: guess/cover/scramble 133, heardle 128, lyric 86.
...
Random lyric line from `lyrics.json` (86 entries) with 1-2 words blanked. Hints after wrong guesses: word length, first letter, album title. Free-text input (not AlbumAutocomplete). Uses `GuessHistory` with custom `checkFn` that normalizes whitespace/punctuation.
