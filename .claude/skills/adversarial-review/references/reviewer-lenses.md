...
The `Map findings to:` names come from the skill author's private principle files, which are
not published with the skill. Treat them as the _category_ of concern a finding belongs to,
and cite this project's own standards (`CLAUDE.md`, the relevant `docs/` file) as the
authority.
